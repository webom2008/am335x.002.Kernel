...
void beep_timer_fn(unsigned long arg)
{
	 ...
	 {
	 	if(data->low_status == ONLOW_NONE)
	 	{
			data->low_status = ONLOW_ON;
			data->cnt = 7;                        //280MS
			pwm_start(data->beep_device);
		} 
		else if(data->low_status == ONLOW_ON && data->cnt <= 0)
		{
			data->low_status = ONLOW_OFF;
			data->cnt = (data->timer_control.lowlevel_time)*25;                    //19720MS
			data->storecnt = data->cnt;                   
			pwm_stop(data->beep_device); 
		}
		else if(data->low_status == ONLOW_OFF && data->cnt <= 0)
		{
			data->low_status = ONLOW_NONE;
		}

		if((data->low_status == ONLOW_OFF) && data->storecnt != (data->timer_control.lowlevel_time)*25)
		{
				data->cnt = (int)((data->timer_control.lowlevel_time)*25 -(data->storecnt - data->cnt));	
				data->storecnt = data->timer_control.lowlevel_time*25;
		}	
		--data->cnt;	
		data->beep_timer.expires =  jiffies + HZ/25;     //40ms软中断一次
	 ...
	 {
		if(data->mid_status == ONMID_NONE)
		{
			data->mid_status = ONMID_ON;
			data->cnt = 5;
			pwm_start(data->beep_device);
		}
		else if(data->mid_status == ONMID_ON && data->cnt <= 0)
		{
			data->mid_status = ONMID_OFF;
			data->cnt = 5;
			pwm_stop(data->beep_device);
		}
		else if(data->mid_status == ONMID_OFF && data->cnt <= 0)
		{
			data->mid_status = ONMID_ON_1;
			data->cnt = 5;
			pwm_start(data->beep_device);
		}
		else if(data->mid_status == ONMID_ON_1 && data->cnt <= 0)
		{
			data->mid_status = ONMID_OFF_1;
			data->cnt = 5;
			pwm_stop(data->beep_device);
		}
		else if(data->mid_status == ONMID_OFF_1 && data->cnt <= 0)
		{
			data->mid_status = ONMID_ON_2;
			data->cnt = 5;
			pwm_start(data->beep_device);
		}
		else if(data->mid_status == ONMID_ON_2 && data->cnt <= 0)
		{
			data->mid_status = ONMID_END;
			data->cnt = (data->timer_control.midlevel_time)*25; 
			data->storecnt = data->cnt;                   //19720MS       
			pwm_stop(data->beep_device);
		}	
		else if(data->mid_status == ONMID_END && data->cnt <= 0)
		{
			data->mid_status = ONMID_NONE;
		}
		if((data->mid_status == ONMID_END) && data->storecnt != (data->timer_control.midlevel_time)*25)
		{
				data->cnt = (int)((data->timer_control.midlevel_time)*25 -(data->storecnt - data->cnt));	
				data->storecnt = (data->timer_control.midlevel_time)*25;
		}	
		--data->cnt;
		data->beep_timer.expires =  jiffies + HZ/25;
		add_timer(&data->beep_timer);

	 }

	 else if(data->alarm_level == ALARM_HIGH)
	 {
	 	if(data->high_status == ONHIGH_NONE)
	 	{
	 		data->high_status = ONHIGH_ON;
			data->cnt = 5;
			pwm_start(data->beep_device);
		}
		else if(data->high_status == ONHIGH_ON && data->cnt <= 0)
		{
			data->high_status = ONHIGH_OFF;
			data->cnt = 1;
			pwm_stop(data->beep_device);
		}
		else if(data->high_status == ONHIGH_OFF && data->cnt <= 0)
		{
			data->high_status = ONHIGH_ON_1;
			data->cnt = 5;
			pwm_start(data->beep_device);

		}
		else if(data->high_status == ONHIGH_ON_1 && data->cnt <= 0)
		{
			data->high_status = ONHIGH_OFF_1;
			data->cnt = 1;
			pwm_stop(data->beep_device);
		}
		else if(data->high_status == ONHIGH_OFF_1 && data->cnt <= 0)
		{
			data->high_status = ONHIGH_ON_2;
			data->cnt = 5;
			pwm_start(data->beep_device);

		}
		else if(data->high_status == ONHIGH_ON_2 && data->cnt <= 0)
		{
			data->high_status = ONHIGH_OFF_2;
			data->cnt = 7;
			pwm_stop(data->beep_device);
		}
		else if(data->high_status == ONHIGH_OFF_2 && data->cnt <= 0)
		{
			data->high_status = ONHIGH_ON_3;
			data->cnt = 5;
			pwm_start(data->beep_device);

		}
		else if(data->high_status == ONHIGH_ON_3 && data->cnt <= 0)
		{
			data->high_status = ONHIGH_OFF_3;
			data->cnt = 1;
			pwm_stop(data->beep_device);
		}
		else if(data->high_status == ONHIGH_OFF_3 && data->cnt <= 0)
		{
			data->high_status = ONHIGH_ON_4;
			data->cnt = 5;
			pwm_start(data->beep_device);

		}
		else if(data->high_status == ONHIGH_ON_4 && data->cnt <= 0)
		{
			data->high_status = ONHIGH_OFF_4;
			data->cnt = 15;
			pwm_stop(data->beep_device);
		}
		else if(data->high_status == ONHIGH_OFF_4 && data->cnt <= 0)
		{
			data->high_status = ONHIGH_ON_5;
			data->cnt = 5;
			pwm_start(data->beep_device);

		}
		else if(data->high_status == ONHIGH_ON_5 && data->cnt <= 0)
		{
			data->high_status = ONHIGH_OFF_5;
			data->cnt = 1;
			pwm_stop(data->beep_device);
		}
		else if(data->high_status == ONHIGH_OFF_5 && data->cnt <= 0)
		{
			data->high_status = ONHIGH_ON_6;
			data->cnt = 5;
			pwm_start(data->beep_device);

		}
		else if(data->high_status == ONHIGH_ON_6 && data->cnt <= 0)
		{
			data->high_status = ONHIGH_OFF_6;
			data->cnt = 1;
			pwm_stop(data->beep_device);
		}
		else if(data->high_status == ONHIGH_OFF_6 && data->cnt <= 0)
		{
			data->high_status = ONHIGH_ON_7;
			data->cnt = 5;
			pwm_start(data->beep_device);

		}
		else if(data->high_status == ONHIGH_ON_7 && data->cnt <= 0)
		{
			data->high_status = ONHIGH_OFF_7;
			data->cnt = 7;
			pwm_stop(data->beep_device);
		}
		else if(data->high_status == ONHIGH_OFF_7 && data->cnt <= 0)
		{
			data->high_status = ONHIGH_ON_8;
			data->cnt = 5;
			pwm_start(data->beep_device);

		}
		else if(data->high_status == ONHIGH_ON_8 && data->cnt <= 0)
		{
			data->high_status = ONHIGH_OFF_8;
			data->cnt = 1;
			pwm_stop(data->beep_device);
		}	
		else if(data->high_status == ONHIGH_OFF_8 && data->cnt <= 0)
		{
			data->high_status = ONHIGH_ON_9;
			data->cnt = 5;
			pwm_start(data->beep_device);

		}
		else if(data->high_status == ONHIGH_ON_9 && data->cnt <= 0)
		{
			data->high_status = ONHIGH_END;
			data->cnt = (data->timer_control.highlevel_time)*25;                    //19720MS 
			data->storecnt = data->cnt;  
			pwm_stop(data->beep_device);
		}
		else if(data->high_status == ONHIGH_END && data->cnt <=0 )
		{
			data->high_status = ONHIGH_NONE;
		}
		
		--data->cnt;	
		if((data->high_status == ONHIGH_END) && data->storecnt != (data->timer_control.highlevel_time)*25)
		{
				data->cnt = (int)((data->timer_control.highlevel_time)*25 -(data->storecnt - data->cnt));	
				data->storecnt = (data->timer_control.highlevel_time)*25;
		}	
		data->beep_timer.expires =  jiffies + HZ/25;               //大概40秒进一次
		add_timer(&data->beep_timer);
...
static void beep_timer_init(void)
{
	...
	beep_control.beep_timer.expires = jiffies + HZ/1000;       //1ms 进入定时器函数  减少不必要的延时
				 ...
				 const char *buf, size_t len)
{
	...
	{
		if(beep_control.alarm_level != ALARM_LOW)             //级别改变
		{
			del_timer(&beep_control.beep_timer);
			beep_control.high_status = ONHIGH_NONE;
			beep_control.mid_status = ONMID_NONE;
	...
	{
		if(beep_control.alarm_level != ALARM_MID)
		{
			del_timer(&beep_control.beep_timer);
			beep_control.high_status = ONHIGH_NONE;
			beep_control.low_status = ONLOW_NONE;
	...
	{
		if(beep_control.alarm_level != ALARM_HIGH)
		{
			del_timer(&beep_control.beep_timer);
			beep_control.low_status = ONLOW_NONE;
			beep_control.mid_status = ONMID_NONE;
